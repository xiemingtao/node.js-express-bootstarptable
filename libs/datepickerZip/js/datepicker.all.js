...
$(function () {
  ...
  $.extend(Time.prototype, {
    ...
    event: function () {
      ...
      // 点击选择时分秒
      this.picker.$container.on('click', '.c-datepicker-time-spinner__item ', function (e) {
        var _this = API.getPicker($(this), 'time');
        var index = _this.picker.$container.find('.c-datepicker-scrollbar__wrap').index($(this).parents('.c-datepicker-scrollbar__wrap'));
        // 兼容滚动延时，导致下一个时间滚动清除掉上一个的timer,所以分开是三个
        clearTimeout(timerArr['timer' + index]);
        timerArr['timer' + index] = setTimeout(function () {
          var index = _this.picker.activeTimeWrap.find('.c-datepicker-scrollbar__wrap').index($(this).parents('.c-datepicker-scrollbar__wrap'));
          var $time = _this.picker.activeTimeWrap.find('.c-datePicker__input-time');
          var day = _this.picker.activeTimeWrap.find('.c-datePicker__input-day').val();
          var val = $time.val();
          val = val.split(':');
          val[index] = API.fillTime(e.target.innerHTML);
          val = val.join(':');
          $time.val(val);
          if (!_this.picker.config.isRange) {
            // 更新input val
